...
export class VendorService {
    ...
    // REFACTORED: FETCH VENDOR REQUESTS (PAGINATED)
    static async fetchVendorRequests(page: number = 1, limit: number = 10) {
        const result = await paginate<IVendor>(
            Vendor,
            { isVerified: false, isPending: true },
            {
                page,
                limit,
                sort: { createdAt: -1 },
        ...
        return { 
            success: true, 
            message: result.data.length ? "Vendors fetched successfully" : "No pending requests found", 
            meta: result.meta,
            vendors: result.data 
        };
    }
    ...
    // REFACTORED: FETCH VERIFIED VENDORS (PAGINATED)
    static async fetchVerifiedVendors(page: number = 1, limit: number = 10) {
        const result = await paginate<IVendor>(
            Vendor,
            { isVerified: true },
            {
        ...
        return { 
            success: true, 
            message: result.data.length ? "Vendors fetched successfully" : "No verified vendors found", 
            meta: result.meta,
            vendors: result.data 
        };
    }
