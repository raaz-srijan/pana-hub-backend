...
export class BookController {
    ...
    static getVerifiedBooks = catchAsync(async (req: Request, res: Response) => {
        const result = await BookService.fetchVerifiedBooks();
        return res.status(200).json(result);
    });
    ...
    static getUnVerifiedBooks = catchAsync(async (req: Request, res: Response) => {
        const result = await BookService.fetchUnVerifiedBooks();
        return res.status(200).json(result);
    });
    ...
    //Expecting query strings, e.g., /books/search?name=hobbit OR /books/search?isbn=12345
    ...
    //FETCH SOFT DELETES (View Trash bin)
